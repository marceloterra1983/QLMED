      ...
      {!loading && details && (
        <div className="space-y-5">
          <CollapsibleCard
            ...
            subtitle="Dados fiscais e endereço do destinatário na última NF-e emitida"
          ...
          <CollapsibleCard
            title="Dados Gerais"
            subtitle="Resumo consolidado das vendas para este cliente"
            open={isGeneralOpen}
            onToggle={() => setIsGeneralOpen((prev) => !prev)}
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
              <StatCard label="NF-e emitidas" value={details.purchases.totalInvoices.toLocaleString('pt-BR')} icon="receipt_long" />
              <StatCard label="Total vendido" value={formatCurrency(details.purchases.totalValue)} icon="payments" />
              <StatCard
          ...
          <CollapsibleCard
            ...
            subtitle="Histórico por item com base nas NF-e emitidas para este cliente"
          ...
          <CollapsibleCard
            ...
            subtitle="Histórico de notas fiscais emitidas para este cliente"
            open={isInvoicesOpen}
            onToggle={() => setIsInvoicesOpen((prev) => !prev)}
          >
            {details.invoices.length === 0 ? (
              <div className="px-4 py-10 text-center text-slate-400 text-sm">Nenhuma nota fiscal encontrada para este cliente.</div>
            ) : (
          ...
          <CollapsibleCard
            ...
            subtitle="Parcelas encontradas nas notas fiscais deste cliente"
            open={isDuplicatesOpen}
            onToggle={() => setIsDuplicatesOpen((prev) => !prev)}
          >
            {details.duplicates.length === 0 ? (
              <div className="px-4 py-8 text-center text-slate-400 text-sm">Nenhuma duplicata encontrada para este cliente.</div>
            ) : (
              <div className="overflow-x-auto max-h-[320px]">
    ...
    <>
      {inline ? (
        ...
        <Modal
          isOpen={isOpen}
          onClose={onClose}
          title={details?.customer.name || customer?.name || 'Visualizar cliente'}
          width="max-w-6xl"
        >
          {content}
        </Modal>
