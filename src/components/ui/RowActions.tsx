...
export default function RowActions({ invoiceId, onView, onDetails, onDelete }: RowActionsProps) {
  ...
  return (
    <div className="flex items-center justify-center gap-0.5">
      ...
      <button
        onClick={() => onDetails(invoiceId)}
        className="p-2 rounded-lg text-slate-500 hover:text-primary hover:bg-primary/10 transition-colors"
        title="Detalhes da NF-e"
        aria-label="Detalhes da NF-e"
      >
      ...
      <div className="relative" ref={menuRef}>
        ...
        {open && (
          <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xl shadow-slate-200/50 dark:shadow-black/30 z-50 py-1.5 animate-in fade-in slide-in-from-top-1 duration-150">
            {menuItems.map((item, i) => (
              <div key={item.label}>
                {item.danger && <div className="my-1.5 h-px bg-slate-200 dark:bg-slate-700" />}
                <button
                  onClick={item.action}
                ...
                >
                  <span className={`material-symbols-outlined text-[18px] ${item.danger ? '' : 'text-slate-400 dark:text-slate-500'}`}>
                    {item.icon}
                  </span>
                  {item.label}
                </button>
