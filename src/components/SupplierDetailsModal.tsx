...
export default function SupplierDetailsModal({
  ...
  const handleSaveShortName = async () => {
    ...
    try {
      const res = await fetch('/api/contacts/nickname', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cnpj: supplier.cnpj, shortName: shortNameDraft }),
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const SortableHeader = ({ label, sortKey, align = 'left' }: { label: string; sortKey: PriceSortKey; align?: 'left' | 'right' }) => (
    <th className={`${thCls} ${align === 'right' ? 'text-right' : ''}`}>
      ...
      >
        {label}
        <span className={`material-symbols-outlined text-[13px] ${priceSortKey === sortKey ? 'text-orange-500' : ''}`}>{getSortIcon(sortKey)}</span>
      </button>
    </th>
  );
      ...
      {!loading && details && (
        <div className="flex flex-col gap-3">
          ...
          <SectionCard
            ...
            subtitle="Dados fiscais e endereço do emitente"
            icon="badge"
          ...
          <SectionCard
            ...
            subtitle="Histórico por item com base nas NF-e recebidas"
            icon="table_chart"
