...
async function fetchSupplierDetails(targetSupplier: SupplierRef): Promise<SupplierDetailsResponse> {
  ...
  if (!res.ok) throw new Error('Falha ao carregar dados do fornecedor');
  return res.json();
}
...
export default function SupplierDetailsModal({ isOpen, onClose, supplier, inline = false }: SupplierDetailsModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setIsRegistrationOpen(false); setCnpjData(null); setCnpjLoading(false);
      setIsGeneralOpen(true); setIsPriceTableOpen(false); setIsInvoicesOpen(false);
      setIsMovimentacoesOpen(false); setIsDuplicatesOpen(false);
  ...
  useEffect(() => {
    ...
    const loadSupplierDetails = async () => {
      ...
      try {
        ...
        if (!cancelled) toast.error('Erro ao carregar detalhes do fornecedor');
      } finally {
        if (!cancelled) setLoading(false);
      }

      if (!cancelled && supplier.cnpj) {
        try {
          const nickRes = await fetch(`/api/contacts/nickname?cnpj=${encodeURIComponent(supplier.cnpj)}`);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (supplier && isOpen) {
        setLoading(true);
        try { const refreshedDetails = await fetchSupplierDetails(supplier); setDetails(refreshedDetails); }
        catch { toast.error('Erro ao atualizar dados do fornecedor'); }
  ...
  const handleSaveShortName = async () => {
    ...
    try {
      const res = await fetch('/api/contacts/nickname', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ cnpj: supplier.cnpj, shortName: shortNameDraft }) });
  ...
  const cnaeMismatchWarning = useMemo(() => {
    if (!cnpjData?.cnaePrincipal || !details) return null;
    ...
    <>
      ...
      {!loading && details && (
        <div className="flex flex-col gap-3">
          {/* Dados de Cadastro */}
          <SectionCard title="Dados de Cadastro" subtitle="Dados fiscais e endereço do emitente" icon="badge" iconColor="text-orange-500" open={isRegistrationOpen} onToggle={() => setIsRegistrationOpen((prev) => !prev)}>
            {/* Nome abreviado */}
            <div className="flex items-center gap-2 mb-3">
              <span className="material-symbols-outlined text-[14px] text-orange-500">edit_note</span>
              <input type="text" value={shortNameDraft} onChange={(e) => setShortNameDraft(e.target.value)} placeholder="Nome abreviado (ex: Distribuidora XYZ)..." maxLength={60} className="flex-1 px-2 py-1 text-[13px] rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/50 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500/40 focus:border-orange-500 transition-all" />
              <button onClick={handleSaveShortName} disabled={savingShortName || shortNameDraft === shortName} className="flex items-center gap-1 px-2.5 py-1 text-[12px] font-bold bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors disabled:opacity-40 shrink-0">
                {savingShortName && <span className="material-symbols-outlined text-[13px] animate-spin">sync</span>}
                {savingShortName ? '...' : 'Salvar'}
              </button>
            </div>

            <ContactInfoSection contact={details.supplier} contactFiscal={details.contactFiscal} />

            <AddressSection
              contact={details.supplier} contactOverride={contactOverride} cnpjData={cnpjData}
              isEditing={isEditing} editDraft={editDraft} savingOverride={savingOverride}
              accentColor="orange" onToggleEdit={handleToggleEdit} onEditField={handleEditField}
            ...
            {!cnpjLoading && cnpjData && (
              <FiscalSection cnpjData={cnpjData} cnpjLoading={cnpjLoading} onSync={handleSyncCnpj} cnaeMismatchWarning={cnaeMismatchWarning} />
            )}
          ...
          <div className="order-first">
            <SectionCard title="Dados Gerais" subtitle="Resumo consolidado das compras" icon="analytics" iconColor="text-emerald-500" open={isGeneralOpen} onToggle={() => setIsGeneralOpen((prev) => !prev)}>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
                ...
                <StatCard label="Total comprado" value={formatAmount(details.purchases.totalValue)} icon="payments" color="emerald" />
                <StatCard label="Itens comprados" value={details.purchases.totalPurchasedItems.toLocaleString('pt-BR', { maximumFractionDigits: 4 })} icon="shopping_cart" color="indigo" />
                <StatCard label="Produtos comprados" value={details.purchases.totalProductsPurchased.toLocaleString('pt-BR')} icon="inventory_2" color="amber" />
                <StatCard label="Última compra" value={details.purchases.lastIssueDate ? formatDate(details.purchases.lastIssueDate) : '-'} icon="event" color="teal" />
              </div>
            </SectionCard>
          </div>

          {/* Tabela de Preco */}
          <SectionCard title="Tabela de Preço" subtitle="Histórico por item com base nas NF-e recebidas" icon="table_chart" iconColor="text-teal-500" open={isPriceTableOpen} onToggle={() => setIsPriceTableOpen((prev) => !prev)} badge={details.priceTable.length || undefined}>
            <PriceTableSection priceTable={details.priceTable} meta={details.meta} sortAccentColor="text-orange-500" />
          </SectionCard>

          {/* Notas Fiscais - Compras */}
          <SectionCard title="Notas Fiscais" subtitle="Compras e bonificações" icon="receipt_long" iconColor="text-primary" open={isInvoicesOpen} onToggle={() => setIsInvoicesOpen((prev) => !prev)} badge={purchaseInvoices.length || undefined}>
            <InvoiceTable invoices={purchaseInvoices} installmentsMap={invoiceInstallmentsMap} emptyLabel="Nenhuma nota fiscal de compra encontrada" onView={openInvoiceViewer} onDetails={openInvoiceDetails} onDelete={confirmDelete} />
          </SectionCard>
      ...
      {!loading && !details && (
        <div className="flex flex-col items-center justify-center py-16 gap-3">
          <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center ring-1 ring-slate-200/50 dark:ring-slate-700/50">
            <span className="material-symbols-outlined text-[32px] text-slate-300 dark:text-slate-600">storefront_off</span>
          </div>
          <p className="text-[13px] font-medium text-slate-400">Sem dados para este fornecedor</p>
        </div>
      )}
    </>
  );
            ...
            <div className="px-4 sm:px-6 py-4 bg-white dark:bg-card-dark border-b border-slate-200 dark:border-slate-700 shrink-0 shadow-[0_2px_8px_rgba(0,0,0,0.08)] sm:shadow-none">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-orange-500/20 to-orange-500/5 dark:from-orange-500/30 dark:to-orange-500/10 flex items-center justify-center ring-1 ring-orange-500/20 dark:ring-orange-500/30 shrink-0 hidden sm:flex">
                    <span className="material-symbols-outlined text-[22px] text-orange-500">local_shipping</span>
                  </div>
                  <div className="min-w-0">
                    <h3 className="text-[15px] font-bold text-slate-900 dark:text-white leading-tight truncate">
                      {details?.supplier.name || supplier?.name || 'Visualizar fornecedor'}
                    </h3>
                    {details?.supplier.cnpj && (
                      <span className="text-[11px] font-mono text-slate-400 dark:text-slate-500">{formatDocument(details.supplier.cnpj)}</span>
                    )}
                  </div>
                </div>
