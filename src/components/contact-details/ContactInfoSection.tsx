  ...
  return (
    <div className="space-y-1.5 mb-3">
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5 text-xs">
        ...
        {contact.fantasyName && <span className="text-slate-500 dark:text-slate-400 text-xs">({contact.fantasyName})</span>}
      </div>
      <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-xs text-slate-500 dark:text-slate-400">
        ...
        {ie && (
          <span>
            ...
            {ieResult && (
              ieResult.valid
                ? <span className="text-emerald-500 ml-1 text-xs">OK</span>
                : <span className="text-amber-600 ml-1 text-xs" title={ieResult.message}>Irregular</span>
            )}
          </span>
        )}
