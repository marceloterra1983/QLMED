...
import { formatDate, formatAmount, formatInt, formatInvoiceNumber } from '@/lib/utils';
            ...
            <div key={inv.id} className="rounded-lg border border-slate-200 dark:border-slate-800 p-2.5">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-slate-900 dark:text-white">N. {formatInvoiceNumber(inv.number)}</span>
            ...
            {visibleInvoices.map((inv) => {
              ...
              return (
                <tr key={inv.id} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/30 transition-colors">
                  <td className={`${tdCls} text-xs font-bold tabular-nums text-slate-800 dark:text-white`}>{formatInvoiceNumber(inv.number)}</td>
        ...
        {visibleDuplicates.map((dup, index) => {
          ...
          return (
            <div key={`m-${dup.invoiceId}-${dup.invoiceNumber}-${dup.installmentNumber}-${dup.dueDate || 'sem-data'}-${index}`} className="rounded-lg border border-slate-200 dark:border-slate-800 p-2.5">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-slate-900 dark:text-white">{formatInvoiceNumber(dup.invoiceNumber)}</span>
            ...
            {visibleDuplicates.map((dup, index) => {
              ...
              return (
                <tr key={`${dup.invoiceId}-${dup.invoiceNumber}-${dup.installmentNumber}-${dup.dueDate || 'sem-data'}-${index}`} className="hover:bg-slate-50/70 dark:hover:bg-slate-800/30 transition-colors">
                  <td className={`${tdCls} text-xs font-bold tabular-nums text-slate-800 dark:text-white`}>{formatInvoiceNumber(dup.invoiceNumber)}</td>
