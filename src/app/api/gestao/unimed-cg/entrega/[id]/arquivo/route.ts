...
const log = createLogger('gestao/unimed-cg/entrega/:id/arquivo');
...
export async function GET(
  ...
  try {
    ...
    log.info(
      ...
      'PDF Unimed CG entrega pronto para stream',
    );
    ...
    prisma.accessLog
      .create({
        data: {
          ...
          action: 'navigation',
          path: `download unimed-cg-entrega id=${row.id} file=${row.fileName}`,
        },
      })
      .catch((err) => log.warn({ err, authorizationId: row.id }, 'AccessLog unimed-cg entrega pdf write failed'));
    ...
    log.error({ err: error }, 'Falha ao servir PDF Unimed CG entrega');
    return apiError(error, 'gestao/unimed-cg/entrega/:id/arquivo');
  }
}
