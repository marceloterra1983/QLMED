...
const log = createLogger('gestao/unimed-cg/ordem-compra/:id/arquivo');
...
export async function GET(
  ...
  try {
    ...
    log.info(
      { purchaseOrderId: row.id, bytes: content.size, durationMs: Date.now() - startedAt },
      'PDF Unimed CG ordem de compra pronto para stream',
    );
    ...
    prisma.accessLog
      .create({
        data: {
          ...
          action: 'navigation',
          path: `download unimed-cg-oc id=${row.id} file=${row.fileName}`,
        },
      })
      .catch((err) => log.warn({ err, purchaseOrderId: row.id }, 'AccessLog unimed-cg oc pdf write failed'));
    ...
    log.error({ err: error }, 'Falha ao servir PDF Unimed CG ordem de compra');
    return apiError(error, 'gestao/unimed-cg/ordem-compra/:id/arquivo');
  }
}
