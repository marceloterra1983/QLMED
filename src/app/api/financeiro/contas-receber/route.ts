...
function getParcelaGroupKey(duplicata: Pick<ContasReceberDuplicata, 'invoiceId' | 'faturaNumero' | 'nfNumero'>) {
...
export async function GET(req: Request) {
  try {
    ...
    for (const item of baseDuplicatas) {
      ...
      if (
        normalizedSearch &&
        !flexMatch(clienteNome, normalizedSearch) &&
        !clienteCnpj.includes(normalizedSearch) &&
      ...
      const duplicata: ContasReceberDuplicata = {
