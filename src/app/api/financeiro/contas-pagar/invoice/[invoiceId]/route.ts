...
export async function GET(
  ...
  try {
    ...
    const duplicatas: ContasPagarDuplicata[] = expandedDuplicatas.map((item) => {
    ...
    const fallbackEmitenteNome = invoice.direction === 'issued'
      ? (invoice.recipientName || '')
      : (invoice.senderName || '');
    const fallbackEmitenteCnpj = invoice.direction === 'issued'
      ? (invoice.recipientCnpj || '')
      : (invoice.senderCnpj || '');
    const headerEmitenteNome = duplicatas[0]?.emitenteNome || fallbackEmitenteNome;
    const headerEmitenteCnpj = duplicatas[0]?.emitenteCnpj || fallbackEmitenteCnpj;
    ...
    console.error('Error fetching contas pagar invoice details:', error);
