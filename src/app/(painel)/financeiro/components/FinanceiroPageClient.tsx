...
export type FinanceiroUiDirection = 'pagar' | 'receber';

type Config = {
  direction: FinanceiroUiDirection;
  apiBase: string;
  ...
  csvFilename: string;
  partyCnpj: (d: Duplicata) => string | undefined;
  partyNome: (d: Duplicata) => string | undefined;
  headerFromDup: (dup: Duplicata) => InvoiceHeader;
};

const CONFIG: Record<FinanceiroUiDirection, Config> = {
  pagar: {
    direction: 'pagar',
    apiBase: '/api/financeiro/contas-pagar',
    defaultStatusFilter: '',
    title: 'Contas a Pagar',
    subtitle: 'Duplicatas das NF-e recebidas',
    searchPlaceholder: 'Buscar por fornecedor, CNPJ, NF-e...',
    loadError: 'Erro ao carregar contas a pagar',
    csvPartyHeader: 'Fornecedor',
    csvFilename: 'contas-pagar',
    partyCnpj: (d) => d.emitenteCnpj,
    partyNome: (d) => d.emitenteNome,
    headerFromDup: (dup) => ({
      id: dup.invoiceId,
      number: dup.nfNumero,
      issueDate: dup.nfEmissao,
      totalValue: dup.nfValorTotal,
      emitenteNome: dup.emitenteNome,
      emitenteCnpj: dup.emitenteCnpj,
    }),
  },
  receber: {
    direction: 'receber',
    ...
    loadError: 'Erro ao carregar contas a receber',
    csvPartyHeader: 'Cliente',
    csvFilename: 'contas-receber',
    partyCnpj: (d) => d.clienteCnpj,
    partyNome: (d) => d.clienteNome,
    headerFromDup: (dup) => ({
      id: dup.invoiceId,
      number: dup.nfNumero,
      issueDate: dup.nfEmissao,
...
export default function FinanceiroPageClient({ direction }: { direction: FinanceiroUiDirection }) {
  ...
  const handleExport = () => {
    const headers = [cfg.csvPartyHeader, 'CNPJ', 'NF-e', 'Fatura', 'Duplicata', 'Vencimento', 'Valor', 'Status'];
    ...
    link.href = url;
    link.download = `${cfg.csvFilename}-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
  ...
  const openDetails = async (dup: Duplicata) => {
    ...
    setInvoiceHeader(cfg.headerFromDup(dup));
    setEditingDuplicatas([]);
    setIsDetailsOpen(true);
    setLoadingDetails(true);

    try {
      const res = await fetch(`${cfg.apiBase}/invoice/${dup.invoiceId}`);
      if (!res.ok) {
      ...
      const rows: Duplicata[] = Array.isArray(data.duplicatas) && data.duplicatas.length > 0
        ? data.duplicatas
        : [dup];
      setInvoiceHeader(data.invoice || cfg.headerFromDup(dup));
  ...
  const handleSaveDetails = async () => {
    ...
    try {
      const res = await fetch(`${cfg.apiBase}/invoice/${invoiceHeader.id}/installments`, {
        method: 'PUT',
