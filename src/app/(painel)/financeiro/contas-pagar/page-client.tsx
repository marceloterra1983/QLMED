...
export default function ContasPagarPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const cnpjs = Array.from(new Set(loaded.map((d) => d.emitenteCnpj).filter(Boolean))) as string[];
      if (cnpjs.length > 0) {
    ...
    <div className="w-full min-w-0 overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-2 mb-6">
        <div className="hidden sm:block min-w-0">
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">Duplicatas das NF-e recebidas</p>
        </div>
        <button
