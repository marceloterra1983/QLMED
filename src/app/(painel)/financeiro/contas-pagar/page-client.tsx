...
const statusConfig: Record<string, { label: string; classes: string; icon: string; dotClass: string }> = {
  overdue: {
    label: 'Vencida',
...
export default function ContasPagarPage() {
  ...
  const handleExport = () => {
    const headers = ['Fornecedor', 'CNPJ', 'NF-e', 'Fatura', 'Duplicata', 'Vencimento', 'Valor', 'Status'];
  ...
  const openDetails = async (dup: Duplicata) => {
    ...
    try {
      const res = await fetch(`/api/financeiro/contas-pagar/invoice/${dup.invoiceId}`);
      if (!res.ok) {
  ...
  const handleSaveDetails = async () => {
    ...
    try {
      const res = await fetch(`/api/financeiro/contas-pagar/invoice/${invoiceHeader.id}/installments`, {
        method: 'PUT',
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Buscar por fornecedor, CNPJ, NF-e..."
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 text-xs uppercase text-slate-500 dark:text-slate-400 font-bold tracking-wider">
                    ...
                    <th className="px-3 py-2.5 cursor-pointer group hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" onClick={() => handleSort('emitente')}>
                      <div className="flex items-center gap-1">Fornecedor <SortIcon col="emitente" /></div>
                    </th>
                          ...
                          {!collapsedGroups.has(group) && (
                            ...
                            >
                              ...
                              <td className="px-3 py-2">
                                <span className="text-sm font-medium text-slate-900 dark:text-white truncate block max-w-[200px]" title={getNick(dup.emitenteCnpj, dup.emitenteNome).display}>{getNick(dup.emitenteCnpj, dup.emitenteNome).display}</span>
                              </td>
                              <td className="px-3 py-2">
