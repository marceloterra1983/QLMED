...
export default function CustomersPage() {
  ...
  useEffect(() => {
    ...
    // Mark as fetching immediately via ref to prevent re-entry
    for (const customer of missingCustomers) {
      fetchedKeysRef.current.add(getCustomerKey(customer));
    }
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {loading ? (
                ...
                customers.map((customer) => {
                  ...
                  return (
                    <tr key={`${customer.cnpj}-${customer.name}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
