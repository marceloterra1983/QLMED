...
describe('cascata S1..S6', () => {
  ...
  it('S5 similaridade: mesmo fornecedor + NCM + descrição >= 0,85', () => {
    ...
    // fornecedor diferente → não casa por descrição
    expect(matchItem(item({ supplierCnpj: '1', supplierName: 'OUTRO LTDA', supplierCode: 'VMED-7', description: 'EQUIPO PARA GERADOR ULTRASSONICO VIA MEDICAL', ncm: '90189099' }), index)).toBeNull();
  });
