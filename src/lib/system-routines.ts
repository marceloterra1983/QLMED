...
export const SYSTEM_ROUTINES: SystemRoutine[] = [
  {
    ...
    scheduleDetails: 'Checagem a cada 60s no minuto :00 (SEFAZ_AUTO_SYNC_MINUTE); piso default SEFAZ_AUTO_SYNC_INTERVAL_MINUTES=360; cooldown anti-656. Telemetria compartilhada: heartbeat do processo auto-sync.',
  ...
  },
  {
    ...
    scheduleDetails: 'Intervalo em minutos configurado por empresa (padrão 60 min, minuto :00 em NSDOCS_AUTO_SYNC_MINUTE). Telemetria compartilhada: heartbeat do processo auto-sync.',
  ...
  },
  {
    ...
    triggerTypeLabel: 'Agendamento / Timer',
    frequency: 'Periódica configurável',
    scheduleDetails: 'Executado a cada intervalo no minuto :30 (configurável via RECEITA_NFSE_AUTO_SYNC_MINUTE). Telemetria compartilhada: heartbeat do processo auto-sync.',
  ...
  },
  {
    ...
    scheduleDetails: 'Executado a cada ciclo do processo auto-sync. Telemetria compartilhada: heartbeat do processo auto-sync.',
  ...
  {
    id: 'nfe-item-product-link',
    name: 'Vínculo de itens de NF-e recebida ao produto Spica',
    category: 'estoque',
    categoryLabel: 'Estoque & Catálogo',
    triggerType: 'event_driven',
    triggerTypeLabel: 'Gatilho por Evento / Watcher',
    frequency: 'Incremental por nota recebida + varredura sob demanda',
  ...
  },
  {
    ...
    triggerType: 'event_driven',
    triggerTypeLabel: 'Gatilho por Evento / Watcher',
    frequency: 'Sob demanda (POST)',
    scheduleDetails: 'POST /api/products/anvisa/sync-registry e /api/anvisa/validate; não corre no ingest de XML',
    concurrencyLock: 'Sanitização de registro e cache de consultas por empresa',
    sourceModule: 'src/lib/anvisa-api.ts / src/app/api/products/anvisa/sync-registry/route.ts',
    description: 'Valida códigos de registro ANVISA contra a consulta pública, sob ação do operador (sync do cadastro ou validate).',
  },
  {
  ...
  },
  {
    ...
    concurrencyLock: 'Postgres Advisory Lock por empresa (impcgMailIngestLockKey) + salvamento de estado atômico',
  ...
  },
  {
    ...
    concurrencyLock: 'Postgres Advisory Lock por empresa (cassemsMailIngestLockKey) + rastreamento de remetentes autorizados',
  ...
  },
  {
    ...
    description: 'Varre e-mails OPME Unimed CG via Microsoft Graph, abre o link Clique aqui, gera PDF, grava no OneDrive e notifica o grupo WhatsApp Evolution. No mesmo tick, coleta ordens de compra SOULMV de @unimedcg.coop.br e confirmações GTPlan de no-reply@gtplan.net (assunto ordem de compra + PDF). Após emissão NF-e ao CNPJ 03315918000118, cruza patientName com infCpl para marcar processos faturados.',
  ...
  },
  {
    ...
    name: 'Resumo Diário NF-e Emitidas (nativo)',
    category: 'gestao',
    categoryLabel: 'Gestão & Convênios',
    triggerType: 'worker_cron',
    triggerTypeLabel: 'Worker / Cron do Sistema',
    frequency: 'Catch-up a cada 15 min (alvo 18h Campo Grande)',
    scheduleDetails: 'Job nativo startDailyIssuedSummary (tick 60s às 18h America/Campo_Grande) + timer qlmed-daily-summary-catchup OnUnitActiveSec=15min via /api/system/daily-issued-summary',
    concurrencyLock: 'Idempotência por data CG (estado em disco + memória; advisory lock)',
    sourceModule: 'src/lib/daily-issued-summary-job.ts / ops/scripts/qlmed-daily-summary-catchup.sh',
    backgroundServiceName: 'daily-issued-summary',
    environmentVars: ['DAILY_SUMMARY_NATIVE', 'DAILY_SUMMARY_DRY_RUN', 'DAILY_SUMMARY_STATE_DIR', 'DAILY_SUMMARY_WHATSAPP_GROUP_JID', 'QLMED_DISABLE_BACKGROUND_SERVICES'],
    description: 'Consolida NF-e emitidas do dia (cabeçalho só vendas) e envia ao grupo WhatsApp via Evolution; catch-up systemd cobre falha do tick.',
  },
  {
  ...
  },
  {
    ...
    sourceModule: 'scripts/notification-outbox-worker.py / src/app/api/notifications/outbox/claim/route.ts',
