...
export async function runImpcgIngest(
  ...
  const errors: string[] = [];
  /**
   * A mesma mensagem chega pelas duas caixas no mesmo tick. Entre uma e outra o
   * marcador durável ainda pode não estar visível, então quem garante uma única
   * tentativa por mensagem por tick é esta lista — o `whatsappSentAt` garante
   * entre ticks.
   */
    ...
    for (const mailbox of IMPCG_MAILBOXES) {
      ...
      try {
        ...
        if (error instanceof GraphMailboxTruncatedError) {
          ...
          log.warn({ mailbox: label, pages: error.pages }, 'impcg_mailbox_truncated');
        } else {
      ...
      for (const message of messages) {
        ...
        const retryNotification = Boolean(
          existingSource
            ...
            && isWithinImpcgNotifyWindow(message.receivedAt),
        );
        ...
        if (existingAuth && !upgrades) {
          ...
          try {
            ...
            log.warn({ mailbox: mailboxLabel(mailbox) }, 'impcg_persist_failed');
            continue;
          }
          skipped += 1;
          continue;
