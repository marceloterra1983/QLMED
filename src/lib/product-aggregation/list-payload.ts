 ...
 * Pure Extract-Method: the per-request aggregation below is intentionally NOT
 * unified with `aggregateProductsFromInvoices` in aggregate.ts — that function
 * paginates through ALL invoices with no cap (used by the batch rebuild job),
 * while this one preserves the route's original MAX_INVOICES /
 * MAX_ISSUED_INVOICES / MAX_IMPORT_INVOICES caps and `Set<string>`-based
 * invoiceIds bookkeeping. Unifying them would be a behavior change, which is
 * out of scope here.
 */

import prisma from '@/lib/prisma';
import { normalizeForSearch, cleanString } from '@/lib/utils';
...
import { normalizeAnvisaRegistration } from './anvisa';
import { buildProductKey, normalizeUnit, type ProductFromXml } from './units';
import { extractProductsFromXml } from './xml-products';
import { XML_BATCH_SIZE, normalizeDescriptionToken, normalizeToken } from './shared';

const log = createLogger('products');

const MAX_INVOICES = 3000;
const MAX_ISSUED_INVOICES_LIST = 3000;
...
export interface ProductsListQueryParams {
  ...
  sort: string;
  order: 'asc' | 'desc';
  useAnvisaLookup: boolean;
  useIssuedNfeLookup: boolean;
...
// Distinct from the exported `AggregatedProduct` above (that one uses
// invoiceCount/lastCountedInvoiceId for the persisted rebuild job; this one
// uses invoiceIds: Set<string> for per-request accumulation) — renamed to
// avoid an ambiguous same-name/different-shape collision in this module.
interface RouteAggregatedProduct {
  key: string;
  code: string;
  description: string;
  ncm: string | null;
...
function buildStrictSaleLookupKeysForList(product: {
  code: string | null | undefined;
  unit: string | null | undefined;
  ean?: string | null | undefined;
}) {
...
async function enrichLastSaleDateForProducts(
  ...
  targetProducts.forEach((product, index) => {
    ...
    const lookupKeys = buildStrictSaleLookupKeysForList({
      code: product.code,
      unit: product.unit,
      ean: product.ean,
    });
...
/**
 * Builds the full /api/products list response payload: 3-pass aggregation
 * (received invoices, import-CFOP entries, resale deductions), ANVISA
 * enrichment, product-registry merge, search/filter, sort, and pagination.
 * Extracted verbatim (no logic changes) from the former GET handler body in
 * src/app/api/products/route.ts so it is callable in-process by other
 * modules (e.g. 12-03's sync-anvisa fix) without an HTTP round-trip.
 */
export async function buildProductsListPayload(companyId: string, params: ProductsListQueryParams) {
  ...
  const productMap = new Map<string, RouteAggregatedProduct>();
  ...
  if (sort === 'lastSale') {
    await enrichLastSaleDateForProducts(companyId, products);
  }
  ...
  if (sort !== 'lastSale') {
    await enrichLastSaleDateForProducts(companyId, paginatedProducts);
  }
