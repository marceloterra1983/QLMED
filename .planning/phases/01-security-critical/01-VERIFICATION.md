...
| 6 | Health endpoint gates memory/uptime/integrity behind auth; build public; password policy min(6) | VERIFIED | `src/app/api/health/route.ts` calls `getServerSession(authOptions)`. Public response includes status/db/build/timestamp. Authenticated response adds uptime/memory/integrity. Password Zod schema at `users/[id]/route.ts` uses `min(6)`, no `min(4)` found. |
...
| `src/app/api/health/route.ts` | Tiered response | VERIFIED | Public includes `build`; `getServerSession()` gates uptime/memory/integrity |
...
| SEC-05 | 01-03 | Health tiered response | SATISFIED | Public `build` (commitSha); session gates uptime/memory/integrity |
...
**Expected:** Unauthenticated response has `status`, `db`, `build`, `timestamp`. Authenticated response adds `uptime`, `memory`, `integrity`.
