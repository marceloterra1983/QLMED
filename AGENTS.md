...
- Work in task-specific isolated git worktrees on a feature branch. Do not create
  permanent sibling folders (`app-ai-governance` or similar). There is no
  persistent Next preview on this host — do not start `qlmed-dev-preview`,
  bind `:3002`, or recreate `/home/marce/qlmed/.worktrees/preview`.
...
- `/home/marce/qlmed/app/` — checkout canônico com Git (alias `app-dev` → mesmo tree)
  ...
  - `npm run dev` usa a porta **3000** e mata o que estiver nela; a porta 3001
    ...
    resolver o conflito. Não subir Next persistente (`:3002` / unit
    `qlmed-dev-preview` foram retirados). Feature worktrees continuam
    válidas; UI valida em `https://app.qlmed.com.br` após deploy, não num
    preview local permanente.
...
- App: `https://app.qlmed.com.br/` (local: 13000 produção `127.0.0.1` only, 3000
  se alguém subir `npm run dev` no checkout; sem preview persistente)
...
### Preview DEV — retirado

Não há preview persistente neste host. Não criar `qlmed-dev-preview.service`,
não bindar `:3002`, não recriar `/home/marce/qlmed/.worktrees/preview`.
UI valida em `https://app.qlmed.com.br` depois do deploy.
...
- Produção pública: `https://app.qlmed.com.br`. Sem Next persistente no
  server (preview `:3002` retirado).
