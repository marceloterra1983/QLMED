...
for invoice_type in NFE CTE; do
  ...
  env_file="/home/marce/${env_dir}/.env"
  if [[ ! -r "$env_file" ]]; then
...
worker_tmp="$(mktemp /home/marce/notification-outbox-worker.py.XXXXXX)"
...
{
  printf '%s\n' "$filtered"
  echo '*/10 * * * * /usr/bin/python3 /home/marce/notification-outbox-worker.py --invoice-type NFE --env /home/marce/nfe-notify/.env >> /home/marce/nfe-notify/cron.log 2>&1'
  echo '*/10 * * * * /usr/bin/python3 /home/marce/notification-outbox-worker.py --invoice-type CTE --env /home/marce/cte-notify/.env >> /home/marce/cte-notify/cron.log 2>&1'
  echo '15 0 * * * /usr/bin/find /home/marce/nfe-notify/cron.log /home/marce/cte-notify/cron.log -type f -size +10M -exec /usr/bin/truncate -s 0 {} \; 2>/dev/null'
} | awk 'NF && !seen[$0]++' | crontab -

python3 -m py_compile /home/marce/notification-outbox-worker.py
