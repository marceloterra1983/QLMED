...
assert.equal(gate.EXPECTED_MIGRATION, '20260918120000_expand_money_float_decimal');
assert.ok(
...
assert.ok(
  gate.EXPECTED_MIGRATIONS.some((m) => m.name === '20260906230000_invoice_query_performance_indexes'),
  'pin inclui invoice_query_performance_indexes',
);
assert.ok(
  gate.EXPECTED_MIGRATIONS.some((m) => m.name === '20260907030000_invoice_watermark_idx'),
  'pin inclui invoice_watermark_idx',
);
assert.ok(
...
assert.ok(
  gate.EXPECTED_MIGRATIONS.some((m) => m.name === '20260908180000_unimed_cg_ordem_compra'),
  'pin inclui unimed_cg_ordem_compra (SPEC-069)',
);
assert.ok(
  gate.EXPECTED_MIGRATIONS.some((m) => m.name === '20260911120000_company_document_manufacturer'),
  'pin inclui company_document_manufacturer (SPEC-042 cartas)',
);
assert.ok(
  gate.EXPECTED_MIGRATIONS.some((m) => m.name === '20260918120000_expand_money_float_decimal'),
  'pin inclui expand_money_float_decimal (SPEC-004 T009)',
);
